...
int main() {
	...
	cout << "------------------------" << endl;
	cout << "Spam Loaded!. Using username " << j["username"].get<string>() << ", With owner " + j["ownerUsername"].get<string>() << endl;
	...
	bot.SERVER_HOST = j["ip"].get<string>();
	bot.SERVER_PORT = j["port"].get<int>();
